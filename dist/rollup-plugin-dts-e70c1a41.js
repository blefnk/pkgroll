...
var require$1 = (
			false
				? /* @__PURE__ */ cn.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('rollup-plugin-dts-e70c1a41.js', document.baseURI).href)))
				: require
		);
...
function cacheConfig([fromPath, toPath], config) {
    ...
    while (fromPath !== toPath &&
        ...
        fromPath !== g__namespace.dirname(fromPath)) {
        fromPath = g__namespace.dirname(fromPath);
...
function getCompilerOptions(input, overrideOptions, overrideConfigPath) {
    ...
    let dirName = g__namespace.dirname(input);
    let dtsFiles = [];
...
function createPrograms(input, overrideOptions, tsconfig) {
    ...
    for (let main of input) {
        ...
        main = g__namespace.resolve(main);
...
function getCodeFrame() {
    ...
    try {
        ...
        try {
            const esmRequire = cn.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('rollup-plugin-dts-e70c1a41.js', document.baseURI).href)));
            ...
            for (const fileName of Object.keys(chunk.modules)) {
                ...
                for (const ref of allFileReferences.get(fileName.split("\\").join("/")) || []) {
                    if (ref.startsWith(".")) {
                        ...
                        const absolutePathToOriginal = g__namespace.join(g__namespace.dirname(fileName), ref);
                        const chunkFolder = (options.file && g__namespace.dirname(options.file)) ||
                            (chunk.facadeModuleId && g__namespace.dirname(chunk.facadeModuleId)) ||
                            ".";
                        let targetRelPath = g__namespace.relative(chunkFolder, absolutePathToOriginal).split("\\").join("/");
                        if (targetRelPath[0] !== ".") {
    ...
    return {
        ...
        options(options) {
            ...
            if (!Array.isArray(input)) {
                ...
                for (const filename of input) {
                    ...
                    if (g__namespace.isAbsolute(filename)) {
                        name = g__namespace.basename(name);
                    }
                    else {
                        name = g__namespace.normalize(name);
                    }
        ...
        resolveId(source, importer) {
            ...
            if (tsconfig) {
                ...
                const resolvedSource = source.startsWith(".") ? g__namespace.resolve(g__namespace.dirname(importer), source) : source;
            ...
            if (!respectExternal && resolvedModule.isExternalLibraryImport) {
                ...
                return { id: g__namespace.resolve(resolvedModule.resolvedFileName) };
            }
        },
